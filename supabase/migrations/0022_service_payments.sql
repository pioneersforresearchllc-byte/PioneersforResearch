...
--
-- Written idempotently (if exists / if not exists) so a partially-applied
-- run can simply be re-run — the SQL editor executes the whole file as one
-- transaction, so one "already exists" error otherwise blocks the rest.

alter table service_requests add column if not exists final_price_cents int;
...
drop policy if exists service_requests_select_self on service_requests;
...
alter table payments add column if not exists service_request_id uuid
