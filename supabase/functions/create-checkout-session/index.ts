...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Without this, a throw (e.g. a rejected Stripe call) escapes as a
    // runtime 500 with no CORS headers, which the browser blocks — leaving
    // the client with only an opaque "failed to send a request".
...
async function handle(req: Request): Promise<Response> {
  ...
  if (code) {
    ...
    // The code must include this course among its targets.
