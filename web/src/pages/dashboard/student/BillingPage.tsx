...
function InvoiceCard({ inv, studentName, bankDetails, onChanged }: { inv: StudentInvoice; studentName: string; bankDetails: string; onChanged: () => void }) {
  const { t } = useLanguage()
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-border-2 bg-white p-3.5 shadow-sm">
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-3">
        {(invoices ?? []).map((inv) => (
          <InvoiceCard key={inv.id} inv={inv} studentName={profile?.name ?? ''} bankDetails={bankDetails} onChanged={refresh} />
        ))}
      </div>
    </div>
  )
